...
class Encoder(nn.Module):
    ...
    def forward(self, obs_traj):
        ...
        obs_traj_embedding = self.spatial_embedding(obs_traj.reshape(-1, 2))    # 1. embedding
        obs_traj_embedding = obs_traj_embedding.view(
            -1, batch, self.embedding_dim
        )
        state_tuple = self.init_hidden(batch)                                   # (ht,ct) zeros
        output, state = self.encoder(obs_traj_embedding, state_tuple)           # 2. LSTM
        final_h = state[0]
        return final_h
...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        self.seq_len = seq_len      # pred traj len
    ...
    def forward(self, last_pos, last_pos_rel, state_tuple, seq_start_end):
        ...
        pred_traj_fake_rel = []
        decoder_input = self.spatial_embedding(last_pos_rel)                    # 1. embedding
        decoder_input = decoder_input.view(1, batch, self.embedding_dim)

        for _ in range(self.seq_len):
            output, state_tuple = self.decoder(decoder_input, state_tuple)      # 2. decoder
            rel_pos = self.hidden2pos(output.view(-1, self.h_dim))              # 3. hidden2pos
            ...
            decoder_input = self.spatial_embedding(embedding_input)             # for-loop: get traj_real
            decoder_input = decoder_input.view(1, batch, self.embedding_dim)
